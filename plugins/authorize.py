...
async def setup(bot):
    ...
    async def authorize(interaction: discord.Interaction, user: discord.Member):
        if user.id not in bot.config["authorized_users"]:
            ...
            bot.save_config()
            return await bot.discord.messages.send(f"<:Sorted:1495837069996720249> User {user.display_name} has been authorized", response=True)
        else:
            return await bot.discord.messages.send(f"<:Unsorted:1495837051235598346> User {user.display_name} is already authorized", response=True)
    ...
    async def deauthorize(interaction: discord.Interaction, user: discord.Member):
        if user.id in bot.config["authorized_users"]:
            ...
            bot.save_config()
            return await bot.discord.messages.send(f"<:Sorted:1495837069996720249> User {user.display_name} has been deauthorized", response=True)
        else:
            return await bot.discord.messages.send(f"<:Unsorted:1495837051235598346> User {user.display_name} isnt authorized", response=True)
