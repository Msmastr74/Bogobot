...
def build_system_prompt(ai: "AICore[Any, Any]", instruction_text: str) -> str:
    ...
    prompt += f"- `{_tool_use_event}{open_system_tag('attached_metadata')}...{close_system_tag('attached_metadata')} JSON{close_system_tag('event_history')}` records a previous tool or command call in history, optionally with metadata for the Discord message produced by that tool. Use it as history only; do not output event-history blocks.\n"
