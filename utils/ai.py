...
class AICore(Generic[ContextT, ActionT]):
    ...
    def system_prompt(
        ...
        return (
            ...
            f"- `{open_system_tag('attached_metadata')}...{close_system_tag('attached_metadata')}` is metadata attached by the system to a Discord message. It contains message id, time, user metadata, and `perm_level` from the bot account system as a rank name such as `basic`, `authorized`, `mod`, `admin`, or `owner`. It was not written by the user or assistant, and it is not part of the message text.\n"
