...
class AICore(Generic[ContextT, ActionT]):
    ...
    def _system_prompt(
        ...
        return (
            ...
            f"- `{open_system_tag('attached_metadata')}...{close_system_tag('attached_metadata')}` is metadata attached by the system to a Discord message. It contains message id, time, and user metadata. The user metadata is formatted `user_id user_name quoted_display_name`. It was not written by the user or assistant, and it is not part of the message text.\n"
