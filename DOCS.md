...
Visual archive scanning is exposed through `/archive_scan`. It accepts a day (`YYYY-MM-DD`, epoch seconds, epoch milliseconds, or Discord timestamp) and an image attachment, then returns the closest matching archive timestamp as both raw epoch seconds and a Discord timestamp. The scan first locates plausible scale/location candidates, then samples the archive at 1-second intervals. The final scan is parallelized with `SCAN_WORKERS = 3` in `utils.video_archival`; that constant is intentionally internal rather than a config field. To avoid overlapping expensive video scans, the command rejects while a scan is running and for 30 seconds after the previous scan finishes. The command also exposes `min_score`, `locator_interval_seconds`, and `max_candidates` for tuning difficult matches, while the final scan sample interval remains fixed at 1 second.
...
- `/archive_scan date image [min_score] [locator_interval_seconds] [max_candidates]`: Searches a day's visual archive for an attached image crop and returns the matching epoch/Discord timestamp. `date` accepts `YYYY-MM-DD`, epoch seconds, epoch milliseconds, `<t:...>`, or `<t:...:*>`. Only one scan may run at a time, and scans have a 30-second cooldown after finishing.
