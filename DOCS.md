...
Visual archive scanning is exposed through `/archive_scan`. It accepts an image attachment and an optional day (`YYYY-MM-DD`, epoch seconds, epoch milliseconds, or Discord timestamp), defaulting to today when `date` is omitted, then returns the closest matching archive timestamp as both raw epoch seconds and a Discord timestamp, plus the matched archive frame image. `start_time` and `end_time` can restrict the scan window; they accept epoch seconds, epoch milliseconds, Discord timestamps, `HH:MM`, or `HH:MM:SS` on the selected archive day. The scan first locates plausible scale/location candidates from sparse timestamp-based archive frames, then scans actual archived frames and scores fixed candidate regions by visual pixel difference. The final scan is parallelized with `SCAN_WORKERS = 3` in `utils.video_archival`; that constant is intentionally internal rather than a config field. To avoid overlapping expensive video scans, the command rejects while a scan is running and for 30 seconds after the previous scan finishes. The command exposes `min_score` and `max_candidates` for tuning difficult matches, while scan timing stays internal.
