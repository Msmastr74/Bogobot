...
Visual archive scanning is exposed through `/archive scan`. It accepts an image attachment and an optional day (`YYYY-MM-DD`, epoch seconds, epoch milliseconds, or Discord timestamp), defaulting to today when `date` is omitted, then edits a Components v2 progress view with a progress bar while the scan runs. The final view returns the closest matching archive timestamp as both raw epoch seconds and a Discord timestamp, plus the matched archive frame image. `start_time` and `end_time` can restrict the scan window; they accept epoch seconds, epoch milliseconds, Discord timestamps, `HH:MM`, or `HH:MM:SS` on the selected archive day. The scan first locates plausible scale/location candidates from sparse timestamp-based archive frames, then scans actual archived frames and scores fixed candidate regions by visual pixel difference. To avoid overlapping expensive video scans, the command rejects while a scan is running and for 30 seconds after the previous scan finishes. The command exposes `min_score` and `max_candidates` for tuning difficult matches, while scan timing stays internal.
...
- `/archive retrieve time`: Extracts a visual archive frame. `time` accepts epoch seconds, epoch milliseconds, `<t:...>`, or `<t:...:*>`.
- `/archive scan image [date] [min_score] [max_candidates] [start_time] [end_time]`: Searches a day's visual archive for an attached image crop and returns the matching epoch/Discord timestamp plus the matched frame. `date` defaults to today and accepts `YYYY-MM-DD`, epoch seconds, epoch milliseconds, `<t:...>`, or `<t:...:*>`. `start_time` and `end_time` restrict the scan window and accept epoch/Discord timestamps or local day times like `14:30` and `14:30:05`. Only one scan may run at a time, and scans have a 30-second cooldown after finishing.
