...
- `/archive_scan image [date] [min_score] [max_candidates]`: Searches a day's visual archive for an attached image crop and returns the matching epoch/Discord timestamp. `date` defaults to today and accepts `YYYY-MM-DD`, epoch seconds, epoch milliseconds, `<t:...>`, or `<t:...:*>`. Only one scan may run at a time, and scans have a 30-second cooldown after finishing.
