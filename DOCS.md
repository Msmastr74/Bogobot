...
- `/manage ai action:config`: Shows an ephemeral AI management panel. Admins can toggle AI on/off, edit `ai.custom_instruction_text`, toggle scheduled AI breaks, and edit break active/break minutes.
- `/manage ai action:memory`: Shows an ephemeral AI memory portal for channel history and persistent memories. Availability depends on `ai.manage.memory.channel` and `ai.manage.memory.persistent`.

`/manage ai` intentionally does not expose API keys, provider URLs, model settings, request interval, or AI history path/budget settings; those remain config-file only for safety.
